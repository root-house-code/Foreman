...
import { amortization, equityStats } from "./mortgage.js";
...
export function buildAlerts(
  ...
  // ── Mortgage: PMI cancellation opportunity (loan-to-value at/below 80%) ──
  if (budget?.mortgage) {
    const am = amortization(budget.mortgage, now);
    if (am.hasLoan && am.currentBalance > 0) {
      const eq = equityStats(am.currentBalance, budget.mortgage.homeValue);
      if (eq && eq.pmiRemovable) {
        alerts.push({
          id: "mortgage:pmi", kind: "mortgage", severity: "info",
          title: "You may be able to cancel PMI",
          sub: `${Math.round(eq.ltv * 100)}% loan-to-value · ≤ 80%`,
          date: null, detail: "review",
          nav: { page: "mortgage" },
        });
      }
    }
