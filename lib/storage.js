...
// Detect a native file backend: Electron (electron/preload.cjs via contextBridge)
// or the Android app in standalone mode (android/ shell, which injects the same
// window.foreman surface with hasNativeStorage set). In a plain browser build
// window.foreman is undefined, so the idb path is used — unless the page was
// served by a Foreman LAN host, detected in storageInit(), in which case
// reads/writes go to the host over HTTP (remote mode). The Android app in
// connected mode injects window.foreman WITHOUT storage functions, so it falls
// through to remote mode like any other LAN client.
const _file = typeof window !== "undefined" && (window.foreman?.isElectron || window.foreman?.hasNativeStorage) ? window.foreman : null;
...
function _pairingRequired() {
  // Inside the Android app, offer the native mode chooser (re-scan the QR)
  // instead of a dead end — the phone browser case still just shows the text.
  const repair = window.foreman?.isAndroid
    ? `<button onclick="window.foreman.openModeSettings()" style="background:transparent;border:1px solid #8b7d6b;border-radius:4px;color:#c9a96e;cursor:pointer;font-family:inherit;font-size:0.85rem;margin-top:0.6rem;padding:0.5rem 1.1rem">Re-pair with your desktop</button>`
    : "";
