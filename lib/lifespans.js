 ...
 * `fieldVals` is accepted for future material/fuel-based refinement; the curated
 * table already keys roofs by material, so no branching is required today.
 */
export function expectedYears(itemName, fieldVals) {
  return EXPECTED_LIFESPAN[itemName]?.[0] ?? null;
}

/** Full [typical, low, high] tuple, or null if the item has no curated lifespan. */
export function expectedRange(itemName) {
  return EXPECTED_LIFESPAN[itemName] ?? null;
}
