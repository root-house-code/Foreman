...
export function saveChoreCompletionRecord(choreId, date, details) {
  ...
  // TOTAL chore time; each assignee is credited that full time (no split).
...
// Permanently remove a completion record. Returns the new records map.
export function deleteChoreCompletionRecord(key) {
  const records = loadChoreCompletionRecords();
  const next = { ...records };
  delete next[key];
  storageSet(RECORDS_KEY, next);
