...
export const BUILT_IN_TYPES = [
  ...
  { id: "exterior",   label: "Exteriors",  behaviorClass: "spatial",    parentId: null,     builtIn: true },
...
export function migrateStructureCategories() {
  const FLAG = "foreman-structure-retired-v1";
  if (storageGet(FLAG)) return;
  const overrides = storageGet("foreman-category-types");
  if (overrides && typeof overrides === "object") {
    const REMAP = { structure: "system", doors: "system", windows: "system", siding: "system", gutters: "exterior" };
    const updated = Object.fromEntries(
      Object.entries(overrides).map(([cat, tid]) => [cat, REMAP[tid] ?? tid])
    );
    storageSet("foreman-category-types", updated);
  }
  // Also clean structure-typed entries from the entity types store itself.
  const entityData = storageGet("foreman-entity-types");
  if (entityData && Array.isArray(entityData.types)) {
    const RETIRED = new Set(["structure", "doors", "windows", "gutters", "siding"]);
    entityData.types = entityData.types.filter(t => !RETIRED.has(t.id));
    entityData.types = entityData.types.map(t =>
      t.parentId && RETIRED.has(t.parentId) ? { ...t, parentId: "system" } : t
    );
