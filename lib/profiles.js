...
// Foreman used to force three fixed profiles (foreman/default/new) into every
// install; they're now ordinary user profiles like any other, and a fresh
// install ships with none until the user creates one (or the setup wizard,
// once it exists, creates the first one for them). This metadata is kept only
// for migrateLegacyBuiltInProfiles() below, to preserve continuity for
// installs that already have data under these keys.
const LEGACY_PROFILE_META = [
  {
    key: "foreman",
    label: "Foreman",
...
export function deleteUserProfile(key) {
  ...
  if (key === "foreman") {
    // No other implicit fallback exists for "foreman" itself — reset to a
    // clean slate instead of switchProfile("foreman"), which would just
    // re-snapshot and restore the data this call is meant to delete.
    storageDelMany(PROFILE_DATA_KEYS);
    storageSet("foreman-active-profile", "foreman");
    storageFlushNow();
    window.location.reload();
  } else {
    switchProfile("foreman");
  }
}
...
export function switchProfile(targetKey) {
  ...
  // Flush synchronously before the reload. Storage writes are debounced (~100ms)
  // and, in the Electron/LAN delta path, sent to the authoritative store via an
  // async IPC message; an immediate reload tears the renderer down before that
  // send fires, so the new active profile and snapshot would be lost and the
  // profile switch/creation silently no-ops. deleteUserProfile and
  // importProfileData already flush for this same reason.
  storageFlushNow();
  window.location.reload();
}
...
// ─── Migration: legacy built-in profiles → user profiles ─────────────────────
// One-time, idempotent — call at boot (see src/main.jsx). Preserves continuity
// for installs that already have data under the old foreman/default/new keys;
// a fresh install has none, so nothing gets created and the app ships clean.
const LEGACY_MIGRATED_KEY = "foreman-legacy-profiles-migrated";

function _hasRealProfileData(key) {
  const data = getProfileData(key);
  if (!data) return false;
  return Object.values(data).some(v => v != null);
}

export function migrateLegacyBuiltInProfiles() {
  if (storageGet(LEGACY_MIGRATED_KEY)) return;
  storageSet(LEGACY_MIGRATED_KEY, true);

  const existing = loadUserProfiles();
  const existingKeys = new Set(existing.map(p => p.key));
  const toAdd = LEGACY_PROFILE_META.filter(
    meta => !existingKeys.has(meta.key) && _hasRealProfileData(meta.key)
  );
  if (toAdd.length === 0) return;

  saveUserProfiles([
    ...existing,
    ...toAdd.map(meta => ({ ...meta, isUser: true })),
  ]);
}
