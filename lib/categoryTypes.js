...
import { loadEntityTypes, getBehaviorClass, resolveTypeId, isExteriorType } from "./entityTypes.js";

const KEY = "foreman-category-types";

// Compatibility: GROUP_ORDER preserved for pages that still iterate over it.
export const GROUP_ORDER = ["room", "system", "exterior", "safety"];

// Fallback types for built-in seed categories. Used when a row's categoryType
// field is absent (e.g. older stored data). User overrides always take precedence.
export const BUILT_IN_CATEGORY_TYPES = {
  "HVAC":                    "system",
  "Plumbing":                "system",
  "Electrical":              "system",
  ...
  "Laundry":                 "room",
  "Attic":                   "exterior",
  "Basement & Crawl Space":  "exterior",
  "Bathrooms":               "room",
  "Garage":                  "exterior",
  "Exterior":                "exterior",
  "Gutters & Drainage":      "exterior",
  "Roof":                    "exterior",
  "Yard & Landscaping":      "exterior",
  "Pest Control":            "safety",
...
// All "location" categories — both rooms AND exteriors (behaviorClass "spatial",
// since the built-in `room` and `exterior` types are both spatial). Returns
// { list, extSet } where extSet flags which entries are exteriors, so callers can
// route an assignment to exteriorLabel vs roomLabel. Mirrors Inventory's
// authoritative item-detail Location picker: the FULL category taxonomy, typed by
// `override ?? row.categoryType` (never silently defaulting rooms to "system"),
// not just categories that happen to have items.
export function loadLocationCategories() {
  const rows = loadData();
  const overrides = loadCategoryTypeOverrides();
  const deletedCategories = loadDeletedCategories();
  const entityData = loadEntityTypes();
  ...
  const list = [], extSet = new Set();
  Object.keys(typeMap).forEach(cat => {
    const typeId = resolveTypeId(cat, overrides[cat] ?? typeMap[cat]);
    if (getBehaviorClass(typeId, entityData) === "spatial") {
      list.push(cat);
      if (isExteriorType(typeId, entityData)) extSet.add(cat);
    }
  });
  return { list: list.sort(), extSet };
}
