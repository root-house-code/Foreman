...
// Payment cycle — how often the utility is billed. Months-per-cycle normalizes a
// bill amount to a monthly figure (mirrors services' toMonthly).
export const UTILITY_BILLING_CYCLES = [
  ["monthly",    "Monthly"],
...
export function utilityMonthsPerCycle(cycle) { return CYCLE_MONTHS[cycle] || 1; }
export function utilityCycleLabel(cycle) {
  return (UTILITY_BILLING_CYCLES.find(([v]) => v === cycle) || UTILITY_BILLING_CYCLES[0])[1];
}
...
// Estimated monthly cost for one utility: the average bill within the trailing 12
// months divided by the utility's billing-cycle length (so a $200 every-2-months
// bill reads as $100/mo, a $1,200 annual bill as $100/mo). Falls back to the
// typical monthly amount, then 0.
export function estimatedMonthly(utility, billsForUtility) {
  ...
  if (recent.length > 0) {
    const avgBill = recent.reduce((s, b) => s + (Number(b.amount) || 0), 0) / recent.length;
    return avgBill / utilityMonthsPerCycle(utility?.billingCycle);
  }
