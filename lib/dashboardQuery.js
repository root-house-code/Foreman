...
export const DATA_SOURCES = {
  ...
  "chore-completions": {
    ...
    fields: [
      ...
      { field: "durationMinutes", label: "Minutes (per person)", type: "number" },
      { field: "month",           label: "Month",               type: "derived" },
      { field: "year",            label: "Year",                type: "derived" },
    ],
  },
...
function loadChoreCompletionRecords() {
  ...
  Object.entries(raw).forEach(([key, entry]) => {
    if (!entry) return;
    // Expand each completion into one row PER assignee so grouping by assignee
    // is per-person. Total time is divided equally → durationMinutes per person.
    const assignees = Array.isArray(entry.assignees) && entry.assignees.length
      ? entry.assignees
      : (entry.assignee ? [entry.assignee] : [""]);
    const totalMin = parseDurationMinutes(entry.duration);
    const perPerson = assignees.length > 0 ? totalMin / assignees.length : totalMin;
...
export function runQuery(config = {}) {
  ...
  const records  = getDataSource(source);
