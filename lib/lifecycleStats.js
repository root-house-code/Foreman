...
export function buildRoster(itemFieldValues, inventory) {
  ...
  rows.forEach(row => {
    ...
    const life = parseFloat(vals.estimated_lifespan);
    ...
    items.push({
      ...
      estimatedLifespan: isNaN(life) ? null : life,
...
// Items with an expected lifespan (the item's own value, else the type default)
// and a usable date, aged and sorted soonest-first. `lifespanDefaults` is the
// type-level default map (Default Values page) used as the fallback per item.
export function computeForecast(roster, now = new Date(), lifespanDefaults) {
  const out = [];
  roster.forEach(it => {
    const exp = it.estimatedLifespan ?? expectedYears(it.item, lifespanDefaults);
