...
export const DEFAULT_MORTGAGE = {
  ...
  escrowMonthly: null,
  overrides: {},
  // Optional loan terms — drive amortization (balance, payoff, interest, equity) in
  // lib/mortgage.js. Additive: the cash-flow model still runs off defaultMonthly.
  principal: null,    // original loan amount
  rate: null,         // annual interest rate, percent
  termYears: null,    // loan term in years (e.g. 30)
  startMonth: null,   // "YYYY-MM" of the first payment
  homeValue: null,    // current home value, for equity / LTV / PMI
};
...
// Mark a planned item as reconciled with a ledger expense.
export function markPlannedLogged(ym, id, expenseId) {
  const b = loadBudget();
  const forMonth = (b.planned[ym] || []).map(p => p.id === id ? { ...p, expenseId } : p);
  return saveBudget({ ...b, planned: { ...b.planned, [ym]: forMonth } });
}
