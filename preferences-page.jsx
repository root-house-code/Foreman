...
// Coerce any legacy/imported string entries into { id, name } objects. A member
// stored as a bare string (e.g. from a hand-authored import) would otherwise have
// an undefined .name and crash the whole Settings page at member.name.slice(...).
function normalizePeople(raw, prefix) {
  return (Array.isArray(raw) ? raw : []).map((m, i) =>
    (m && typeof m === "object") ? m : { id: `${prefix}-legacy-${i}`, name: String(m ?? "") }
  );
}

function loadMembers() {
  try { return normalizePeople(storageGet(HOUSEHOLD_MEMBERS_KEY), "m"); }
  catch { return []; }
}
...
function loadServiceProviders() {
  try { return normalizePeople(storageGet(HOUSEHOLD_PROVIDERS_KEY), "sp"); }
  catch { return []; }
}
...
function ProfileSettings() {
  ...
  function buildSnap() {
    // Values must be PARSED (arrays/booleans), not JSON strings: createProfile
    // stores this snap and switchProfile writes each value straight into live
    // storage without parsing — matching snapshotToStorage's in-memory format.
    // (The stringified form is only for the export FILE, which importProfileData
    // parses back.) Stringifying here would leave foreman-chores as the string
    // "[]" — which loadChores returns verbatim, breaking every chores consumer —
    // and foreman-use-default-data as the truthy string "false", so unchecking
    // "seed inventory" would silently still seed it.
    const snap = {};
    for (const k of PROFILE_DATA_KEYS) snap[k] = null;
    snap["foreman-chores"]           = [];
    snap["foreman-todos"]            = [];
    snap["foreman-projects"]         = [];
    snap["foreman-use-default-data"] = seedInventory;

    if (seedInventory && !seedTasks) {
      const taskKeys = defaultData
        .filter(r => r.category && r.item && r.task)
        .map(r => `${r.category}|${r.item}|${r.task}`);
      snap["foreman-deleted-rows"] = taskKeys;
    }
    return snap;
  }
